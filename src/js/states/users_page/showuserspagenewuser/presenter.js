const stateShowUsersPageNewUserPresenter = async (message) => {
    debug(`stateShowUsersPageNewUserPresenter()`)
    const model = await stateShowUsersPageNewUserModel(message)
    ...
    waitForElement({ "selector": saveFormBtn }, () => {
        ...
        document.querySelector(saveFormBtn).addEventListener('click', () => {
            ...
            stateShowUsersPageNewUserSave(currentState, dataForm)
        })
    })
}
